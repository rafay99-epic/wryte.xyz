/**
 * Non-Node database helpers for `aiCredentials`.
 *
 * Mirrors `convex/mediaCredentialsDb.ts` exactly — split out so the Node-only
 * action file can call into these via `ctx.runQuery` / `ctx.runMutation`.
 */
