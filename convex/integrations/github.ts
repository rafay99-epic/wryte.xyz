...
/**
 * Commits one generated animation .tsx into the target repo.
 *
 ...
 */
async function commitAnimationComponent(
  octokit: Octokit,
  opts: {
    owner: string;
    repo: string;
  ...
  try {
    ...
    if (!Array.isArray(data) && data.type === "file") {
      ...
      if (existingContent === opts.fileContent) return; // up to date
  ...
  await octokit.repos.createOrUpdateFileContents({
    ...
    path: opts.repoPath,
    message: existingSha
      ? `Update animation ${opts.name}`
      : `Add animation ${opts.name}`,
...
export const publishToGithub = internalAction({
  ...
  handler: async (ctx, args) => {
     ...
     *    directives) and commit every referenced .tsx component BEFORE the
     *    post that imports them, so the site never builds against a
     *    missing file. The author writes plain `<Anim />`; everything
     *    framework-specific is generated here. */
    ...
    if (
      ...
      if (animationRows.length > 0) {
        ...
        // ponytail: sequential per-file commits, not an atomic tree commit.
        // A mid-sequence failure leaves already-committed components with
        // no referencing post — harmless unused files. Switch to the
        // bulkPublish tree API (createBlob/createTree) if atomicity or
        // commit noise ever matters.
        for (const comp of transformed.components) {
          await commitAnimationComponent(octokit, {
            owner,
            repo,
            branch,
